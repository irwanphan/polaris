...
> **Last updated:** 2026-06-14 by Cursor AI session — **Event Detail route + deep-link infra shipped.** New `/events/:id` route pushes above the bottom-nav shell with a hero countdown card, recurrence + pin badges, note + widget-message sections, and a Pin / Edit / Delete affordance set. A new `core/deep_links/` module (pure `DeepLinkRouter` URI resolver + `PolarisDeepLinkHandler` widget) funnels four launch surfaces — home-screen widget cold start, widget warm taps, notification cold start, notification warm taps — through a single canonical mapper (`polaris://events/<id>` → `/events/<id>`). Native Android: widget per-row fill-in intents now carry the deep-link URI; the PendingIntent template uses `HOME_WIDGET_LAUNCH_ACTION` so the `home_widget` plugin captures the URI on launch. `NotificationDispatcher` interface widened with `tapPayloads` + `consumeColdStartPayload`. Recommendation Engine v2 + Multi-Pin Widget (v3) shipped earlier. **215 total tests passing** (was 195), `flutter analyze` clean, debug APK installed + verified end-to-end on emulator: widget tap (cold *and* warm) lands on the Event Detail page with the right event.
...
## 2. Current Status (As of 2026-06-14, **M0–M2 + CI + M4 + M5 + M6 + Widget Pin v2 + Multi-Pin Widget (v3) + Recommendation Engine v2 + Event Detail Route + Deep-Link Infra shipped**)
...
| 2026-06-14 | Cursor AI session | Shipped **Event Detail Route + Deep-Link Infrastructure**: (1) **Detail page** — new `EventDetailPage` (consumer of new `eventByIdProvider`, a `Provider.autoDispose.family<AsyncValue<Event?>, String>` derived from `eventsStreamProvider`) renders a hero countdown card (accent-tinted, `displayMedium` number + unit + next-occurrence date), wrap of pill badges (`Repeats <X>` / `One-off` + `Pinned to widget`), three `SectionCard` blocks (KEJADIAN BERIKUTNYA / CATATAN / PESAN WIDGET with italic fallback copy), and a meta footer (Dibuat / Diubah dates); AppBar carries a pin toggle and overflow menu (Edit → opens existing `EventEditorSheet`, Delete → confirm dialog → repo delete; the page auto-pops once the event disappears from the stream via a one-shot `_navigatedAway` guard). (2) **Route wiring** — `/events/:id` registered as a child route of the events `StatefulShellBranch` (pushes above the bottom-nav shell, keeps Acara tab highlighted, back arrow returns to list); `AppRoutes.eventDetail(id)` helper centralises path building. Event list row tap switched from "open editor sheet" → `context.push(detailPath)`. (3) **Deep-link plumbing** — new `core/deep_links/` module: pure `DeepLinkRouter` exposes `resolve(Uri?) → String?`, `eventDetailUri(id) → Uri`, and `notificationPayloadToUri(String?) → Uri?` (backwards-compat: bare uuid payloads from the existing notification scheduler are auto-wrapped to `polaris://events/<id>`); `PolarisDeepLinkHandler` (mounted under `MaterialApp.router.builder`) subscribes to `HomeWidget.widgetClicked` + `notificationDispatcher.tapPayloads` for warm taps and drains `HomeWidget.initiallyLaunchedFromHomeWidget()` + `notificationDispatcher.consumeColdStartPayload()` on first frame for cold starts. Navigates via `router.go(events) → microtask → router.push(detailPath)` so the branch swap happens before the detail push (no wrong-tab flash). (4) **Notification dispatcher widened** — abstract interface now exposes `tapPayloads` (broadcast `Stream<String?>`) + `consumeColdStartPayload()`; the Flutter Local Notifications impl plugs the new plugin v18 `onDidReceiveNotificationResponse` into the broadcast controller and reads `getNotificationAppLaunchDetails()` once for cold start. Test mocks in `widget_test.dart`, `notification_scheduler_test.dart`, `life_pin_controller_test.dart` updated with `Stream<String?>.empty()` + `async => null`. (5) **Android widget native bridge** — `PolarisWidgetItemsFactory.getViewAt` per-row fill-in intent now carries `data = polaris://events/<id>` (uri-encoded); `PolarisWidgetProvider` template intent switched from `Intent.ACTION_VIEW` → `HomeWidgetLaunchIntent.HOME_WIDGET_LAUNCH_ACTION` (mandatory for the home_widget plugin to detect the launch URI) + `FLAG_MUTABLE` on S+ so per-row fill-ins are allowed. (6) **Cold-start GoRouter guard** — when the OS hands Flutter `state.uri.scheme = 'polaris'` directly (the URI bypasses the `home_widget` plugin and goes through the `Router.routeInformationProvider` initial route), the router-level `redirect` re-runs the URI through `DeepLinkRouter.resolve` so the location lands on `/events/<id>` instead of the `GoException: no routes for location: polaris://events/...` page. Sharp edges noted: (a) Riverpod 3 dropped `valueOrNull` on `AsyncValue` — use `async.value` (nullable) and `hasValue` instead; (b) Riverpod 3's `ProviderFamily` is internal and not exported, so explicit type annotations on `Provider.autoDispose.family<…>(…)` won't compile — drop the annotation and let inference do it; (c) collection-widget PendingIntent templates need `data = null` so the fill-in's URI wins under the framework's template+fill-in merge; (d) MainActivity `launchMode="singleTop"` was already set in M2 — good, that's what makes warm taps invoke `onNewIntent` rather than re-launching. **215 total tests** (was 195): new `deep_link_router_test.dart` (12 cases), `event_by_id_provider_test.dart` (5 cases), `event_detail_page_test.dart` (3 cases). `flutter analyze` clean. **End-to-end verified on Pixel API 34 emulator**: (i) list tap → detail opens; (ii) widget cold tap (force-stopped app) → app boots straight into the detail page for the right event; (iii) widget warm tap (app backgrounded) → same; (iv) AppBar back returns to events list; (v) Edit overflow opens the editor sheet pre-filled. Indonesian copy: `Acara` / `Berulang Bulanan` / `Tersemat ke widget` / `HARI` / `KEJADIAN BERIKUTNYA` / `Ubah` / `Hapus`. |
| 2026-09-20 | Cursor AI session | Shipped **P0 — Pastel Theme System + Enhanced Life Hero UI + Widget Palette Support**: (1) **Switchable pastel themes** — new `ColorPalette` enum (Midnight / Blush / Lavender / Mint / Peach) with full 50–950 scales in `PaletteTokens`. `ThemeController` uses `Notifier` pattern (matches `LocaleController`); `ThemePreferences` immutable state triggers MaterialApp rebuild + widget refresh on change. `AppTheme` now builds per-palette `ColorScheme`s; all widgets read from `ColorScheme` so they auto-adapt. (2) **Settings integration** — new `ThemePickerTile` shows horizontal scrollable palette cards with color swatches; uses `Material` + `InkWell` for proper tap feedback (splash). (3) **Enhanced Life hero** — countdown card now has soft radial gradient background (primary @ 8% opacity), subtle drop shadow, gradient shader on hero number (primary → secondary), bolder font (700), secondary text in pill badge, new `_LifeProgressChips` component (days lived + % lived badges, days mode only), xl2 radius (24px) + low-opacity border. (4) **Widget palette support** — `PolarisHomeWidgetUpdater` passes palette primary/secondary colors via SharedPreferences; `PolarisWidgetProvider` applies to header + brand pill; `PolarisWidgetItemsFactory` applies accent color to icon backgrounds; distinct icons (hourglass for life, calendar for events); hero number increased to 24sp, title smaller (11sp); widget refreshes automatically when palette changes. (5) **l10n** — added `lifePercentLived` (EN: "lived", ID: "telah dijalani"). PR https://github.com/irwanphan/polaris/pull/1 (branch `cursor/p0-pastel-themes-hero-ui-6256`). `flutter analyze` clean. Verified on AVD: palette picker tappable with splash feedback, in-app UI + widget update instantly, distinct icons render. |
