...
| 2026-09-20 | Cursor AI session | Shipped **P0 — Pastel Theme System + Enhanced Life Hero UI**: (1) **Switchable pastel themes** — new `ColorPalette` enum (Midnight / Blush / Lavender / Mint / Peach) with full 50–950 scales in `PaletteTokens`. `ThemeController` persists palette + brightness in SharedPreferences, exposes reactive stream. `AppTheme` now builds per-palette `ColorScheme`s; all widgets read from `ColorScheme` so they auto-adapt. (2) **Settings integration** — new `ThemePickerTile` shows horizontal scrollable palette cards with color swatches; integrated under APPEARANCE section. (3) **Enhanced Life hero** — countdown card now has soft radial gradient background (primary @ 8% opacity), subtle drop shadow, gradient shader on hero number (primary → secondary), bolder font (700), secondary text in pill badge, new `_LifeProgressChips` component (days lived + % lived badges, days mode only), xl2 radius (24px) + low-opacity border. Feels "gemas" (charming/cute) with soft pastels and elevation. (4) **l10n** — added `lifePercentLived` (EN: "lived", ID: "telah dijalani"). PR opened on branch `cursor/p0-pastel-themes-hero-ui-6256`. Tests deferred until `build_runner` regenerates `theme_controller.g.dart` on local machine. `flutter analyze` expected clean. Ready for emulator smoke test. |
